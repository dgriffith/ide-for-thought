...
- Query the graph for existing terms: `query_graph` with `SELECT ?t ?term WHERE { ?t a/rdfs:subClassOf* thought:Term . OPTIONAL { ?t thought:term ?term } }` — the property path matches terms typed either the current way (`type: glossary-term` frontmatter) or the older way (an embedded `` ```turtle this: a thought:Term . ``` `` block), so existing entries in either convention still turn up. This tells you which terms already exist and where they live.
- `read_note` one or two existing term notes to confirm the directory (normally `glossary/`) and the frontmatter schema (`term`, `aliases`, `disambiguation`, `see-also`). Also check how they're typed: `type: glossary-term` in the frontmatter (current convention), or a closing `` ```turtle this: a thought:Term . ``` `` block (older convention, from before the Glossary Term object type existed).
...
- The same body skeleton: H1 term, one-line working definition, optional expanded paragraph, optional `## Not to be confused with`, optional `## See also`.
