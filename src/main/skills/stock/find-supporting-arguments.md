...
<the inferential chain — "X because Y because Z, so the original claim follows">
...
The frontmatter URI is the load-bearing piece — that's what materialises the `thought:supports` triple. **Use the literal claim URI as the value** (not a wiki-link, not a paraphrase): the indexer recognises bare `https://…` values as IRI nodes.
