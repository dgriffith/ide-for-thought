...
4. **Preserve the note's machinery.** Keep any YAML frontmatter, existing `[[wiki-links]]`, and `#tags` intact unless the user asked to change them. `propose_note_body` REPLACES the whole file, so each edit's `content` must include the frontmatter and links, not just the prose.
5. **Propose the rewrite.** Call `propose_note_body` ONCE with a single entry in `edits` — `relative_path` set to the note's path and `content` set to the complete new markdown. (If the user asked you to fill out several notes, put them all in `edits` on that one call: the batch becomes one review card and one all-or-nothing change.) Optionally pass a one-line `note` describing what you did (e.g. "Expanded the dictated outline into sections").
