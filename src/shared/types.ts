...
// Conversation core types (Conversation, ConversationMessage, ContextBundle,
// Citation, TurnUsage, …) moved to ./conversation.ts (#1912) — this was the
// one section of this file that was a dump rather than a catalog. Re-export
// removed deliberately: every consumer now imports from ./conversation
// directly, matching how every other shared/conversation-*.ts domain module
// is already consumed.
//
// PrivilegedSite (per-machine cookie-partition state) sat inside this section
// too, despite having nothing to do with conversations — moved to its own
// ./privileged-sites.ts, owned on the main side by main/privileged-sites.ts.
