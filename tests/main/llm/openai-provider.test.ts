...
describe('OpenAIProvider — history shaping', () => {
  ...
  it('compactToolUseInputs is a no-op with an empty id set', () => {
    const assistant = [{ role: 'assistant', content: null, tool_calls: [] }] as unknown as ReturnType<typeof provider.ingestHistory>[number];
    expect(provider.compactToolUseInputs(assistant, new Set(), {})).toBe(assistant);
  });
});
